...
/**
 * Lambda adn inv_Lambda are only the diagonal elements
*/
// __global__
...
// {
//     int tid = blockIdx.x * blockDim.x + threadIdx.x;
//     Map<VectorXd> Lambda_eigen(Lambda, n_comb);
//     Lambda_eigen.setOnes();
//     Map<VectorXd> inv_Lambda_eigen(inv_Lambda, n_comb);
//     if (tid < n_comb)
//     {
//         // double beta = pow( 1 + pow(2 * epsilon / alpha, 2), 0.25 );
//         // double delta = alpha * alpha / 2 * (beta * beta - 1);
//         for (int p = 0; p < n_dim; p++)
//         {
//             Lambda_eigen(tid) *= sqrt( alpha*alpha / (alpha*alpha + delta + epsilon*epsilon) ) \
//                     * pow( epsilon*epsilon / (alpha*alpha + delta + epsilon*epsilon), eigenvalue_comb[tid + p * n_comb] - 1 );
//         }
//         inv_Lambda_eigen(tid) = 1 / Lambda_eigen(tid);
