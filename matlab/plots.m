...
if exist('__octave_config_info__', 'builtin')
    ...
    log_cpu = readmatrix("../log_cpu_2080.csv", 'FileType', 'text', 'Delimiter', ',');
    log_cpu = log_cpu(:, 1:5);
    log_gpu = readmatrix("../log_gpu_2080.csv", 'FileType', 'text', 'Delimiter', ',');
    log_gpu = log_gpu(:, 1:5);
end
...
for dims = 1:max(log_cpu(:, 4))
    ...
    hold on, grid on
    plot(n_eig_gpu, mean_time_gpu, '--o')
    legend('i7 7700', 'Quadro P2200', 'R5 3600', 'RTX 2080 Super', 'Location','best')
    xlabel("Number of eigenvalues $n$")
    ylabel("Time [ms]")
    xlim([min(min(n_eig_cpu), min(n_eig_gpu)), max(max(n_eig_cpu), max(n_eig_gpu))])
    title([ num2str(dims), " dimensions"])
...
if exist('__octave_config_info__', 'builtin')
    ...
    log_cpu = readmatrix("../log_cpu_1050.csv", 'FileType', 'text', 'Delimiter', ',');
    log_cpu = log_cpu(:, 1:5);
    log_gpu = readmatrix("../log_gpu_1050.csv", 'FileType', 'text', 'Delimiter', ',');
    log_gpu = log_gpu(:, 1:5);
end
...
for dims = 1:max(log_cpu(:, 4))
    ...
    hold on, grid on
    plot(n_eig_gpu, mean_time_gpu, '--o')
    legend('i7 7700', 'Quadro P2200', 'R5 3600', 'RTX 2080 Super', 'i7 7700HQ', 'GTX 1050 Mobile', 'Location','best');
    xlabel("Number of eigenvalues $n$")
    ylabel("Time [ms]")
    xlim([min(min(n_eig_cpu), min(n_eig_gpu)), max(max(n_eig_cpu), max(n_eig_gpu))])
    title([ num2str(dims), " dimensions"])
